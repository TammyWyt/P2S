...
1. **Seal the gas quantum at B1.** `F_res = φ·gasLimit·baseFee` currently REVEALS gasLimit in cleartext → metadata-dependent positioning defeats G1. Fix: charge F_res on a fixed reserved gas quantum OR a sealed gas commitment revealed at B2. Else re-scope G1 to "content-only" and report the metadata residual.
