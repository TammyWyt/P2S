...
def plot_mev_reordering(data: dict, out_path: str) -> None:
    ...
    ax.bar(x_pos, means, width=0.72, color=colors, yerr=stds, capsize=10, edgecolor="white", linewidth=1.2)
    ax.set_xticks(x_pos)
    ax.set_xticklabels(labels)
    ax.set_ylabel("Mean MEV opportunity per block (ETH)", fontsize=22, fontweight='bold')
